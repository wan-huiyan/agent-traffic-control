...
MUTATIONS = [
    ...
    ("the fast path forgets the receipt runner",
     "_FAST_WORDS = (\"pytest\", \"py.test\", \"gate_receipt\")",
     "_FAST_WORDS = (\"pytest\", \"py.test\")"),
    ("the fast path forgets py.test",
     "_FAST_WORDS = (\"pytest\", \"py.test\", \"gate_receipt\")",
     "_FAST_WORDS = (\"pytest\", \"gate_receipt\")"),
    ("inner texts are not memoized (nesting goes exponential again)",
     "    if cmd in memo:\n        return memo[cmd]\n",
     "    if False:\n        return memo[cmd]\n"),
    ("the size cap is removed",
     "    if len(cmd) > _MAX_CHARS:\n        return False",
     "    if False:\n        return False"),
    ("`sh -s` is not read from stdin",
     "        elif a.startswith(\"-\") and not a.startswith(\"--\") and \"s\" in a[1:]:\n            return True\n",
     "        elif False:\n            return True\n"),
    ("a carriage return is part of a word again",
     "            elif c in \" \\t\\r\":\n                self._end_word()",
     "            elif c in \" \\t\":\n                self._end_word()"),
    ("a non-string command reaches the reader",
     "    if not isinstance(cmd, str) or not cmd or not starts_a_leg(cmd):",
     "    if not cmd or not starts_a_leg(cmd):"),
    ("`ps` output is decoded strictly again",
