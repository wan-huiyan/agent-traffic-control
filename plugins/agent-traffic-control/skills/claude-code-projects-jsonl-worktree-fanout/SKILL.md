...
Today's session: user asked *"locate a previous session (about 2 wks ago) where I ask you to help me summarize what we did and our learnings"* in the `schuh_causal_impact` project.

Initial grep of `~/.claude/projects/-Users-huiyanwan-Documents-schuh-causal-impact/*.jsonl` found 7 files, none matching the description. **Looked like the session was deleted.**

Then I expanded to:

```bash
find ~/.claude/projects/ -name "*.jsonl" -newermt "2026-05-13" ! -newermt "2026-05-16" 2>/dev/null \
  | grep -iE "schuh|causal" \
  | grep -v subagents
```

→ 7 matching JSONLs across **5 different project directories** (1 canonical + 4 worktree-namespaced). The target session was in `-Users-huiyanwan-Documents-schuh-causal-impact--claude-worktrees-eloquent-franklin-fe932f/cff8244f-2194-4448-933c-9a93d417b770.jsonl` — never would have been found by searching the canonical dir alone.
...
- The fan-out can be substantial: as of this session, `~/.claude/projects/` had 38 directories matching `schuh|causal` (1 canonical + 37 worktrees). Similar fanout for other heavily-worktreed projects (`barryU` had 3+).
