...
- **This skill is about STRUCTURE, not CONTENT.** It catches wrong dataset/table/column
  ...
  (The frontmatter description used to end by routing to a sibling skill,
  `verify-plan-constants-against-data`. No such skill exists in this repo or any installed
  bundle, so the redirect gave the model nothing to route to — and the description is text
  the model reads every turn. The distinction is kept, the dead name is out of the
  description. It is left in "See also" below, where it sits alongside other references to
  unpublished siblings; that is a repo-wide convention, not something to fix one line at a
  time.)
