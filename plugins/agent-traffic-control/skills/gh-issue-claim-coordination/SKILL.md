...
- [`rule-cited-by-title-proposes-work-the-ledger-says-is-built`](../rule-cited-by-title-proposes-work-the-ledger-says-is-built/SKILL.md) — the preflight table's CLOSED row, generalised past issues: the work you are about to pick up may already be **built and waiting on a human**, recorded in a shared ledger under a status word (`open`) that reads as "not started". Read it when a rule, decision or policy — not an issue — is what sent you at the work
- [`batch-member-cites-a-path-only-a-non-member-adds`](../batch-member-cites-a-path-only-a-non-member-adds/SKILL.md) — the record that would have stopped you may exist, be correct, and sit on a branch `main` does not have. A session built work its owner had declined by name an hour earlier, because the ruling was unlanded. Search unmerged branches too, not just `main`
