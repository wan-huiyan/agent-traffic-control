...
AMC-handover project, session 6 (2026-05-18). Plan B's 18 TDD tasks executed
via `superpowers:subagent-driven-development` in a worktree at
`.claude/worktrees/plan-b-amc-brief-runner` on branch
`worktree-plan-b-amc-brief-runner`.
...
```bash
git push -u origin worktree-plan-b-amc-brief-runner
gh pr create --title "feat(plan-b): amc-brief-runner core backend" ...
# CI green; squash-merge.
```

PR [#9](https://github.com/wan-huiyan/AMC-handover/pull/9) merged at
...
$ git rev-parse worktree-plan-b-amc-brief-runner        # in worktree
e9bb471acdac9d14c2114097f3230e00a11bc493     # Task 11 ← FROZEN

$ git reflog show worktree-plan-b-amc-brief-runner | head -3
e9bb471 worktree-plan-b-amc-brief-runner@{0}: commit: feat(brief-runner): CUSTOM_PARAMETER ...
2dbed0e worktree-plan-b-amc-brief-runner@{1}: commit: feat(brief-runner): inline SQL parametrizer ...
...
Recovery via [PR #14](https://github.com/wan-huiyan/AMC-handover/pull/14)
(squash `d808859`):
...
forensics in
`docs/analysis/2026-05-18-pr-9-stranded-commits.md` of the AMC-handover repo.
...
- AMC-handover [PR #9](https://github.com/wan-huiyan/AMC-handover/pull/9)
  (the truncated original) and
  [PR #14](https://github.com/wan-huiyan/AMC-handover/pull/14)
  (the cherry-pick recovery).
- AMC-handover post-mortem at `docs/analysis/2026-05-18-pr-9-stranded-commits.md`
