...
**Worked example (amc-brief-runner s18b, 2026-05-27):** PR #84 merged
...
```sh
$ gcloud run services describe barry-propensity-pulse --region=us-central1 \
    --project=barry-cdp --format="value(status.latestReadyRevisionName)"
barry-propensity-pulse-00011-w22

$ gcloud builds list --project=barry-cdp --limit=3 \
    --format="table(id,createTime,images[0])"
50826116-...    2026-05-07 10:10 UTC    gcr.io/barry-cdp/barry-propensity-pulse

$ gcloud builds describe 50826116-... --project=barry-cdp \
    --format="value(source.storageSource.bucket,source.storageSource.object)"
barry-cdp_cloudbuild  source/1778148635.480733-c08dc0a63ec245b98158888fdb0fd39e.tgz

$ gcloud storage cp gs://barry-cdp_cloudbuild/source/1778148635...tgz /tmp/build.tgz
...
$ git log --all --oneline --format="%H" -- ...drivers.html | while read sha; do
    git show "$sha:goal_term_enrollment/.../drivers.html" 2>/dev/null \
...
- **v1.2.0 (2026-05-27):** Added "silent pull" variant — `git pull
  ...
  the working tree matches the commit). Source: amc-brief-runner s18b
