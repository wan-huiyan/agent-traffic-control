...
- [`batch-member-cites-a-path-only-a-non-member-adds`](../batch-member-cites-a-path-only-a-non-member-adds/SKILL.md) — the other direction of the same batch check: a member's added comment or tracker row cites a repository path that only a NON-member adds, so the batch merges green and the target branch points at a file it does not have. Overlap asks who touches a file; this asks who NAMES one. Since 1.1.0 it also carries the mirror — a rule, ruling or ledger every session must obey, sitting where the default reader cannot reach it.
- [`conflicted-pr-starts-no-ci-run-push-resolution-first`](../conflicted-pr-starts-no-ci-run-push-resolution-first/SKILL.md) — while the PR is CONFLICTED no CI run exists or can start, so two of its three status fields are non-facts; push the resolution before your local legs finish
