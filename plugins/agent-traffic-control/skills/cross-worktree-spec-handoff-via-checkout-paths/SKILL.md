...
- [`batch-member-cites-a-path-only-a-non-member-adds`](../batch-member-cites-a-path-only-a-non-member-adds/SKILL.md) — the other half of handing a file over: what you wrote is correct and the DEFAULT reader cannot reach it. Outward, a comment or tracker row names a path only a non-member adds; inward, a rule, ruling or ledger everyone must obey sits on a branch. Read it before deciding a record counts as delivered
